class Length
  ...
  def <=>(other_length)
    case @unit
    when :km then @value  <=> other_length.as_kilometers.value
    when :mi then @value  <=> other_length.as_miles.value
    when :nmi then @value <=> other_length.as_nautical_miles.value
    end
  end
